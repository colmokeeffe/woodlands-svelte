<script lang="ts">
    ...
    let identity = $user._id;
    let message = "";

    const woodlands = getContext("Woodlands");

    async function save() {
        let success = await woodlands.updateSettings(firstName, lastName, email, password, identity )
        if (success) {
    ...
    <div class="uk-margin uk-text-left">
        <div class="uk-margin">
            ...
            <div class="uk-inline uk-width-1-1">
                ...
                <input bind:value={firstName} class="uk-input uk-form-large" type="text" name="firstName" placeholder={firstName}>
            </div>
        </div>
        <div class="uk-margin">
